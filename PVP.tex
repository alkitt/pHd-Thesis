...
\caption{\label{fig:PVP:lattice} (a) Unstrained graphene's real space lattice with labeled nearest neighbor vectors ($\vec{\delta_i}$), labeled lattice translation vectors, $\vec{R}_i=n\vec{a}_+ + m \vec{a}_-$ with $m$ and $n$ integers, and black and gray dots representing the A and B sub-lattices, respectively. (b) The Brillouin zone of unstrained graphene with labeled high symmetry points. (c) The positions of the unstrained (black, solid) and strained (brown, dashed) nearest neighbors, and the corresponding $\vec{\delta}_i$ and $\vec{\delta}_i'$ for 20 \% uniaxial strain along the armchair direction.  (d) The unstrained (black, solid) and strained (brown, dashed) Brillouin zone, also for 20 \% armchair uniaxial strain, with labels for the now inequivalent Dirac points. }
\end{figure}
...
For reference, in Figure \ref{fig:PVP:lattice} graphene's unstrained geometry from chapter \ref{chap:TB} is shown with the real space lattice in (a) and the first Brillouin zone in (b).
...
This magnitude of strain is not necessary; all effects discussed here are linear in strain and so persist at much smaller strains.

This change in lattice geometry alters the electron energy dispersion by \emph{both} shifting the position of the atoms \emph{and}  introducing a bond-dependent nearest neighbor hopping amplitude\cite{Hasegawa2006} because of the modified distance between nearest neighbors.
...
As the distance between nearest neighbor atoms changes, so does the nearest neighbor hopping energy.
...
%KEEP GOING HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%MY NOTES ARE KIND OF FUNNY HERE, ENDS UP DEPENDING ON \nabla \vec{u} pg 18 and 19 in soft theory book

The strain-induced shift of the nearest neighbor vectors can be expressed using the Cauchy-Born hypothesis.
In this approximation, the position of the $i$-th atom in the deformed configuration, $\vec{R}_i$, is given with reference to the undeformed one, $\vec{r}_i$, in terms of the deformation field $\vec{u}(\vec{r})$:
\begin{equation*}
  \vec{R}_{i}=\vec{r}_{i}+\vec{u}(\vec{r_i})
\end{equation*}
The strained nearest-neighbor vectors, $\vec{\delta}_{1,2,3}^{\prime}$ are then given approximately by
\begin{align*}
	\vec{\delta}_i^{\prime}(\vec{r})&=\vec{R}_j-\vec{R}_i \\
	                       			&=(\vec{r}_j-\vec{r}_i)+\vec{u}(\vec{r_j})-\vec{u}(\vec{r_i}) \\
	                       			&\simeq\vec{\delta}_i(\vec{r})+(\vec{\delta}_i \cdot \vec{\nabla}) \vec{u}(\vec{r}) \\
	                       			&=(\bm{1}+\bm{\nabla u}) \cdot \vec{\delta}_i \ ,
\end{align*}
where $\bm{1}$ is the two dimensional identity matrix and $\bm{\nabla}\bm{u}$ is the Jacobian of the displacement field known as the displacement gradient tensor:
\begin{align*}
...
It is important to stress that approximating $\bm{\delta}_{i}^{\prime}(\bm{r})\simeq (\bm{1}+\tilde{\bm{\epsilon}})\cdot\bm{\delta }_{i}$ is only valid if the deformation does not involve local rotation ($\tilde{\bm{\omega}}=0$).
